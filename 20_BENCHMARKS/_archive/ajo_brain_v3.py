"""AJO UniBrain v3: GrowBrain IS the router - soft weights gate cell+PSM."""
import time
import numpy as np
...
class UniBrainV3:
    ...
    def sense(self, data, kind, target=None):
        ...
        g = self.grow.step(sv)
        w = g["w"]
        gate = float(w[g["winner"]])
        h, err, z = self.cell.step(uv * (0.5 + 0.5 * gate))
        ...
        return {"y": y, "M": M, "active": int(h.sum()), "winner": g["winner"],
                "regions": g["regions"], "gate": round(gate, 3), "kind": kind}
...
if __name__ == "__main__":
    ...
    print(f"V3_OK steps=20 kinds=4 regions={r['regions']} winner={r['winner']} gate={r['gate']} ms={ms:.2f} active={r['active']}")
